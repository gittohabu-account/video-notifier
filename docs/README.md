...
- アドホック検索は既知履歴を**読まず・書かず**動作するので、定期実行と完全に独立しています
- ヒット結果は以下で自動的に絞られます：
  - **投稿1年以内** のもののみ
  - **最大30件**（サイトは新しい順なので、自然に新しい30件が選ばれる）
  - 投稿日不明のものはスキップ（古い可能性があるため）
- メール件名は `[動画新着][アドホック] 新着 N 件` で区別されます
- ヒット0件のときも **「結果0件でした」確認メール** が届きます（件名: `[動画新着][アドホック] 結果0件`）。内訳付きなので、ヒットしたけど期間外で除外された、なども分かります
- PAT・履歴はスマホの localStorage に保存（外部に送信されません）
- GitHub API への通信は HTTPS で直接行います（中継サーバーなし）

### 上限・期間を変えたい場合

`video_notifier.py` の以下の定数を編集：

```python
ADHOC_MAX_ITEMS = 30        # メールに載せる最大件数
ADHOC_MAX_MONTHS_AGO = 12   # 投稿後何ヶ月以内まで対象とするか
```
